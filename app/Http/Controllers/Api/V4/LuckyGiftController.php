                    ...
                    if ($sander->balance >=$value) {
                       ...
                       try {
                           $txResult = DB::transaction(function () use (
                               $user_id, $value, $row, $gift_name, $channelName, $host_amount, $game_balance, $reward
                           ) {
                               $sanderLocked = User::where('id', $user_id)->lockForUpdate()->first();
                               if (!$sanderLocked || $sanderLocked->balance < $value) {
                                   throw new \RuntimeException('INSUFFICIENT_SENDER_BALANCE');
                               }
                               $luckyLocked = LuckyGiftSetting::where('id', 1)->lockForUpdate()->first();
                               if (!$luckyLocked) {
                                   throw new \RuntimeException('LUCKY_POT_MISSING');
                               }

                               // Debit sender + credit pot BEFORE writing the Gift row.
                               $sanderLocked->balance -= $value;
                               $sanderLocked->save();
                               $luckyLocked->balance += $game_balance;
                               $luckyLocked->save();
                               ...
                               $finalReward = 0;
                               if ($reward != 0 && $luckyLocked->balance > $reward) {
                                   $luckyLocked->balance -= $reward;
                                   $luckyLocked->save();
                                   $sanderLocked->balance += $reward;
                                   $sanderLocked->save();
                                   $finalReward = $reward;
                               }
                               ...
                               return [
                                   'sander'        => $sanderLocked,
                                   'lucky_balance' => $luckyLocked,
                                   'gift'          => $g,
                                   'reward'        => $finalReward,
                               ];
                           });
                       } catch (\Throwable $e) {
                           \Log::warning('LuckyGift Store transaction skipped/failed', [
                               'user_id' => $user_id,
                               'value'   => $value,
                               'error'   => $e->getMessage(),
                           ]);
                       ...
                       // Re-bind originals to keep downstream code unchanged.
                       $sander = $txResult['sander'];
                       $lucky_balance = $txResult['lucky_balance'];
                       $gift = $txResult['gift'];
