...
class UserLiveController extends Controller
{
    ...
    public function HomeIndex(Request $request)
    {
        ...
        // app_home is a read path. Live cleanup must happen through explicit remove/offline flows.
        ...
        $followerCacheKey = $this->prefix . "followerLive_no_cert_{$userId}";
        
        try {
            $cachedFollower = Redis::get($followerCacheKey);
            if ($cachedFollower) {
                $followerLive = $this->stripSensitiveLiveFields(unserialize($cachedFollower));
            } else {
                $followerLive = $this->stripSensitiveLiveFields($this->getFollowerLiveFromDB($userId));
    ...
    public function Store(Request $request)
    {
        ...
        $this->Websoket(array(array(
            ...
            'channelName' => $channelName,
            'code' => '200',
            'channel_type' => strval($request->type),
        )));
    ...
    public function LivesNowIndex(Request $request)
    {
        ...
        $lives = $this->stripSensitiveLiveFields($this->getCachedUnifiedLivePage($page, $perPage));
        $topSourceLives = $page === 1
            ? $lives
            : $this->stripSensitiveLiveFields($this->getCachedUnifiedLivePage(1, $perPage));
        $topLive = $this->stripSensitiveLiveFields($this->extractTopLiveFromPaginator($topSourceLives));
    ...
    public function Delete(Request $request)
    {
        ...
        if($token == "0411f0028cfb768b3a3d96ac3aa37dw3e5"){
            ...
            if($data){
                $targetChannelName = trim((string) ($channelName ?: $data->channelName));
                ...
                if($check_day_time){
                    ...
                    $daytime->live_time = date('Y-m-d');
                    $daytime->save();
                }

                LiveCall::where('host_id', $id)->delete();
                AudienceJoin::where('host_id', $id)->delete();
                Comment::where('reciever_id', $id)->when($targetChannelName !== '', function ($query) use ($targetChannelName) {
                    $query->where('channelName', $targetChannelName);
                })->delete();
                $this->clearHomeLiveCaches($id);
                if ($targetChannelName !== '') {
                    app(\App\Services\AudioRoom\AudioRoomStateService::class)->purgeRoom($targetChannelName);
                }
                $data->delete();
    ...
    private function clearHomeLiveCaches($userId = null)
    {
        ...
        if ($userId) {
            $keys[] = $this->prefix . "followerLive_no_cert_{$userId}";
        }
