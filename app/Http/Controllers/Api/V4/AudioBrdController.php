...
{
    $access_token = $this->requestValue($request, array('access_token'), '');
    $host_id = $this->requestValue($request, array('host_id', 'live_id'), '');
    $channelName = $this->requestValue($request, array('channelName', 'channel_name', 'room_name'), '');
    $response = array();
    ...
    if(empty($host_id) || empty($channelName)){
        array_push($response,array(
            'message'=>'Missing host_id or channelName',
            'code'=>'422',
        ));
        return json_encode($response,JSON_UNESCAPED_UNICODE);
    }
    ...
    {
        list($startDate, $endDate) = $this->businessMonthRange();
        $todayDate = $this->businessTodayDate();
    ...
    public function Kick(Request $request)
    {
        ...
        if($access_token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
            ...
            $check_admin=BrdAdmin::where('user_id',$host_id)->where('admin_id',$kick_by)->where('type',1)->first();
            
            if($kick_by==$host_id || ($check_offical_user && $check_offical_user->kick_power==1) || $check_admin ){
            ...
            $user_by_kick=RedisCacheFunction::UserfindById($kick_by);
               ...
               array_push($websoket_kick,array('message'=>'bd_kick','data'=>$response,'channelName'=>$channelName,'code'=>'200','channel_type' => '20'));
                self::Websoket($websoket_kick);
            return json_encode($response,JSON_UNESCAPED_UNICODE);
            }else{
                $remove_old_call=LiveCall::where('co_host_id',$kick_by)->first();
                if($remove_old_call){
                $remove_old_call->delete();
                }
                $kick=new Kick;
                $kick->user_id=$kick_by;
                $kick->channelName=$channelName;
                $kick->host_id=$host_id;
                $kick->kick_by=$kick_by;
                $kick->save();
                $user_by_kick=RedisCacheFunction::UserfindById($kick_by);
                    array_push($response,array('message'=>'Kick Successfully ','channelName'=>$channelName,'user_id'=>$kick_by,'user_by_kick'=>$user_by_kick->name,'channelName'=>$channelName,'code'=>'200'));
      ...
      // info('HTTP response code For Audio Brd recieve-one: ' . $co);
        if($access_token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
            if (!app(\App\Services\AudioRoom\AudioRoomStateService::class)->acquireActionLock(
                ...
                array_push($response,array('message'=>'Duplicate call request ignored','code'=>'202'));
                return json_encode($response,JSON_UNESCAPED_UNICODE);
            }
              ...
              if (!app(\App\Services\AudioRoom\AudioRoomStateService::class)->acquireActionLock(
                ...
                if (!empty($normalizedReceivers)) {
                    ...
                    if ($transactionResult['first_receiver_id']) {
                        $balance = Gift::where('reciever_id', $transactionResult['first_receiver_id'])
                            ->where('channelName', $channelName)
                            ->sum('value');
