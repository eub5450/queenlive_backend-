        ...
        if($token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
            ...
            if($balance>=$amount){
              if($amount >= $minimumRechargeAmount){
                    $user=User::find($transfer_member_id);
                if($user)
                {
                
                 try {
                DB::transaction(function () use ($user_id, $transfer_member_id, $amount, $transactionId, $setting) {
                    $user = User::where('id', $transfer_member_id)->lockForUpdate()->first();
                    if (!$user) {
                        throw new \RuntimeException('MEMBER_GONE');
                    }
                    // Re-validate portal balance under transaction (defense in depth).
                    $recheckRecharge = \App\Models\PortalRecharge::where('user_id', $user_id)->where('status', 'Approved')->sum('amount');
                    $recheckTransfer = \App\Models\PortalTransfer::where('portal_user_id', $user_id)->sum('amount');
                    $recheckRecall = \App\Models\PortalRecall::where('protal_id', $user_id)->sum('amount');
                    $recheckP2P = \App\Models\ProtalToPTransfer::where('user_id', $user_id)->sum('amount');
                    $recheckP2PR = \App\Models\ProtalToPTransfer::where('portal_user_id', $user_id)->sum('amount');
                    $reBalance = ($recheckRecharge + $recheckP2PR) - ($recheckTransfer + $recheckRecall + $recheckP2P);
                    if ($reBalance < $amount) {
                        throw new \RuntimeException('INSUFFICIENT_PORTAL_BALANCE');
                    }
                    $transfer=new PortalTransfer;
                                ...
                                $this->applyVipRechargeBenefits($user, (int) $amount, $setting);
                                    ...
                                    $transfer->save();
                                     ...
                                     if($user->invited_by){
                                         $invite_by=User::where('id', $user->invited_by)->lockForUpdate()->first();
                ...
                if ($e->getMessage() === 'MEMBER_GONE') {
                    array_push($response, array('message' => 'This Member Not Found', 'code' => '401'));
                } elseif ($e->getMessage() === 'INSUFFICIENT_PORTAL_BALANCE') {
                ...
                \Log::error('Portal Transfer transaction failed', [
                    'user_id' => $user_id,
                    'transfer_member_id' => $transfer_member_id,
                    'amount' => $amount,
                    'error' => $e->getMessage(),
                ]);
                array_push($response, array('message' => 'Transfer failed', 'code' => '500'));
                return json_encode($response, JSON_UNESCAPED_UNICODE);
            }
