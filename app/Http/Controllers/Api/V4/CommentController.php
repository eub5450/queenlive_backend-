...
class CommentController extends Controller
{
    ...
    public function FlyComment(Request $request){
        ...
        if($token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
            ...
            // self::RealTime($global_txt,$roomName,$channelName);
             try {
                $host_uid_v5 = (string) (\App\Models\UserLive::where('channelName',$channelName)->value('user_id') ?? $user_id);
                app(\App\Services\V5\RoomBroadcastService::class)->broadcast(
                    'audio', $channelName, $host_uid_v5,
                    'room.comment.flying',
                    ['message'=>$message,'image'=>$user->profile,'name'=>$user->name,'channelName'=>$channelName],
                    ['actor_user_id'=>$user_id]
                );
             } catch (\Throwable $e_v5) { info('V5 fly broadcast failed: '.$e_v5->getMessage()); }
            ...
            $audience=AudienceJoin::where('channelName',$channelName)->where('user_id',$user_id)->first();
             if($audience){
                     ...
                     //   self::RealTime($global_txt,$roomName,$channelName);
                        try {
                            app(\App\Services\V5\RoomBroadcastService::class)->broadcast(
                                'audio', $channelName, (string) ($audience->host_id ?? '0'),
                                'room.member.entered',
                                $global_txt[0] ?? [],
                                ['actor_user_id'=>$user_id]
                            );
                        } catch (\Throwable $e_v5) { info('V5 entry broadcast failed: '.$e_v5->getMessage()); }
    ...
    public function AudienceLeave(Request $request)
    {
        ...
        if($token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
          ...
          if($data){
            ...
            // broadcast (bd_entry / channel_type 15). Previously AudienceLeave
            ...
            try {
                ...
                if ($leave_user) {
                    ...
                    try {
                        app(\App\Services\V5\RoomBroadcastService::class)->broadcast(
                            'audio', $channelName, (string) ($data->host_id ?? '0'),
                            'room.member.left',
                            $leave_txt[0] ?? [],
                            ['actor_user_id'=>$user_id]
                        );
                    } catch (\Throwable $e_v5) { info('V5 leave broadcast failed: '.$e_v5->getMessage()); }
                }
  ...
  public function AudienceList(Request $request)
    ...
    { 
        ...
        if($token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
            try{
            // 2026-07-03 moderation fix: no duplicate mute rows; fan out to the
            // room's real private channel set (was hardcoded audio => video and
            // multi rooms never received comment-mute realtime); return 200 so
            // the acting client stops reporting a false failure.
            ...
            $data=CommentMute::where('channelName',$channelName)->get();
             try {
                app(\App\Services\AudioRoom\AudioRoomRealtimeService::class)->broadcastLegacyWithRoomScoped([[
                    'event_type' => 'room.comment.muted',
                    'eventType' => 'room.comment.muted',
                    'message' => 'bd_comment_mute',
                    'channelName' => $channelName,
                    'data' => $data,
                    'user_id' => (string)$user_id,
                    'target_user_id' => (string)$user_id,
                    'code' => '200',
                ]], ['source' => 'CommentModeration']);
             } catch (\Throwable $e_v5) { info('V5 mute broadcast failed: '.$e_v5->getMessage()); }
          array_push($response,array('message'=>'Comment Mute Successfully','channelName'=>$channelName,'user_id'=>$user_id,'code'=>'200'));
          return json_encode($response,JSON_UNESCAPED_UNICODE);
            }catch (\Exception $e) {
    ...
    public function CommentMuteRemove(Request $request){
        ...
        if($token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
         ...
         // broadcast-only, so the user stayed muted in DB and on rejoin), fan
         // out to the room's real private channel set, and return 200.
         CommentMute::where('channelName',$channelName)->where('user_id',$user_id)->delete();
         try {
            app(\App\Services\AudioRoom\AudioRoomRealtimeService::class)->broadcastLegacyWithRoomScoped([[
                'event_type' => 'room.comment.unmuted',
                'eventType' => 'room.comment.unmuted',
                'message' => 'bd_comment_unmute',
                'channelName' => $channelName,
                'data' => array(array('user_id'=>(string)$user_id,'channelName'=>$channelName)),
                'user_id' => (string)$user_id,
                'target_user_id' => (string)$user_id,
                'code' => '200',
            ]], ['source' => 'CommentModeration']);
         } catch (\Throwable $e_v5) { info('V5 unmute broadcast failed: '.$e_v5->getMessage()); }
          array_push($response,array('message'=>'Comment Unmute Successfully','channelName'=>$channelName,'user_id'=>$user_id,'code'=>'200'));
          return json_encode($response,JSON_UNESCAPED_UNICODE);
        }else{
           array_push($response,array('message'=>'Unauthorized','code'=>'401'));
            return json_encode($response,JSON_UNESCAPED_UNICODE); 
