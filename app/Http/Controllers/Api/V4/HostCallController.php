                   ...
                   // self::RealTime($response,$roomName,$channelName);
                       try {
                            app(\App\Services\V5\RoomBroadcastService::class)->broadcast(
                                'video', $channelName, (string) $host_id,
                                'video.cohost.invited',
                                ['data'=>$response,'channelName'=>$channelName,'host_id'=>(string)$host_id,'co_host_id'=>(string)$co_host_id],
                                ['actor_user_id'=>$host_id,'target_user_id'=>$co_host_id]
                            );
                       } catch (\Throwable $e_v5) { info('V5 video.cohost.invited failed: '.$e_v5->getMessage()); }
                           ...
                           // self::RealTime($response,$roomName,$channelName);
                              try {
                                   app(\App\Services\V5\RoomBroadcastService::class)->broadcast(
                                       'video', $channelName, (string) $host_id,
                                       'video.room.snapshot',
                                       ['data'=>$response,'channelName'=>$channelName],
                                       ['actor_user_id'=>$host_id]
                                   );
                              } catch (\Throwable $e_v5) { info('V5 video.room.snapshot failed: '.$e_v5->getMessage()); }
			               return json_encode($response,JSON_UNESCAPED_UNICODE);
            		}else{
            	     array_push($response,array('message'=>'Host Live Not Actived','code'=>'401'));
                     return json_encode($response,JSON_UNESCAPED_UNICODE);
    	...
    	$live=UserLive::where('channelName','=',$channelName)->where('user_id',$host_id)->first();
            if($live){
               ...
               // self::RealTime($response,$roomName,$channelName);
                try {
                    app(\App\Services\V5\RoomBroadcastService::class)->broadcast(
                        'audio', $channelName, (string) $host_id,
                        'audio.cohost.invited',
                        ['data'=>$response,'channelName'=>$channelName,'host_id'=>(string)$host_id,'co_host_id'=>(string)$co_host_id],
                        ['actor_user_id'=>$host_id,'target_user_id'=>$co_host_id]
                    );
                } catch (\Throwable $e_v5) { info('V5 audio.cohost.invited failed: '.$e_v5->getMessage()); }
                return json_encode($response,JSON_UNESCAPED_UNICODE);
            	}else{
            	 array_push($response,array('message'=>'User Already In Your Call','code'=>'401'));
                 return json_encode($response,JSON_UNESCAPED_UNICODE);
                        ...
                        $total_gift_sum= $total_gift_coin-$total_withdraw;
			             $today_gift = Gift::where('reciever_id',$host_id)
                           ...
                           // self::RealTime($response,$roomName,$channelName);
                               ...
                               } catch (\Throwable $e_v5) { info('V5 audio.room.snapshot failed: '.$e_v5->getMessage()); }
			               return json_encode($response,JSON_UNESCAPED_UNICODE);
            		}else{
            	     array_push($response,array('message'=>'Host Live Not Actived','code'=>'401'));
                     return json_encode($response,JSON_UNESCAPED_UNICODE);
