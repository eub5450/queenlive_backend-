        ...
        if($access_token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
          ...
          $data=LiveCall::where('host_id',$host_id)->where('channelName',$channelName)->where('co_host_id',$co_host_id)->where('status','pending')->first();
           ...
           $data->delete();
                ...
                array_push($websoket_kick,array('message'=>'Call Request','channelName'=>$channelName,'call_count'=>$call_count,'data'=>$call_list,'code'=>'200','event_type' => 'multi.call.pending_list'));
     ...
     public function AudioCallAccept(Request $request)
    {
        ...
        if($access_token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
          ...
          if($data){
            ...
            $host['camera_status'] = $live->host_camera_status;
            $host['super_mute'] = "0";
            array_push($list, $host);
            
          foreach ($accept_list as $call) {
                              $co_host = User::find($call->co_host_id); if (!$co_host) { $co_host = new \App\Models\User(); } /* BE-8 null guard */
                              $row = array();
         ...
         $total_reward = $total_reward == 0 ? '0' : (string) $total_reward;
             ...
             array_push($websocket_call,array('message'=>'bd_multi_call','data'=>$response,'channelName'=>$channelName,'code'=>'200','event_type' => 'multi.call.updated'));
     ...
     public function Store(Request $request)
    {
      ...
      $host['channelName'] = $channelName;
         ...
         array_push($list,$host);
              ...
              $today_gift = Gift::where('reciever_id', $user_id)
                            ->whereDate('date', now()->toDateString())
                            ->sum('value');
        ...
        \Log::warning('V5 multi host list failed', ['error' => $e->getMessage()]);
    ...
    public function HostCallRemove(Request $request)
    {
      ...
      $set_no = $request->set_no;
        ...
        if($access_token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
              ...
              array_push($websocket_call,array('message'=>'bd_multi_host_call_remove','data'=>$response,'channelName'=>$channelName,'code'=>'200','event_type' => 'room.cohost.cut'));
                self::Websoket($websocket_call);
    ...
    public function UserData(Request $request)
    {
      ...
      $list = array();
            ...
            foreach ($levels as $threshold => [$level, $nextAmount]) {
             ...
             array_push($websocket_call,array('message'=>'bd_multi_call','data'=>$joinresponse,'channelName'=>$channelName,'code'=>'200','event_type' => 'multi.call.updated'));
             self::Websoket($websocket_call);
             array_push($response,array('message'=>'User Data Show Successfully','code'=>'200','data'=>$data,'follow_status'=>$follow_status,'balance'=>$total_data_sum,'agency_name'=>$agency_name,'host_type'=>$host_type,'marchent'=>$data->is_agency,'is_coin_protal_active'=>$data->is_coin_protal_active,'is_vip'=>$data->is_vip,'frame'=>$data->frame,'entry_effect'=>$data->entry));
            return json_encode($response,JSON_UNESCAPED_UNICODE);
    ...
    public function AudioGiftPush(Request $request)
    {
        ...
        $j=0;
              ...
              $data = json_decode($request->getContent(), true); // Assuming the request is sent as JSON
                ...
                if (isset($data['items']) && is_array($data['items'])) {
                   ...
                   // return $row['value'];
                    // P0-B2 FIX (Agent SF 2026-06-28): per-item debit read an
                    // UNLOCKED User::find and saved outside a transaction
                    // (lost-update / double-spend). Atomic locked debit + gift row.
                    $gift = null;
                    try {
                        $gift = DB::transaction(function () use ($user_id, $value, $row, $gift_name, $channelName) {
                        ...
                        // insufficient balance -> skip this gift item entirely.
                        continue;
                    }
                    ...
                    if ($gift) {
                        ...
                        $message = "$sender_name->name sent $value to $receiver_name->name";
                         ...
                         array_push($gift_global_websoket,array('message'=>'bd_global_gift','channelName'=>$channelName,'data'=>$global_txt,'code'=>'200','event_type' => 'gift.global'));
                    ...
                    $i++;
                     if($j==0){
                      ...
                      $giftSenderUser = isset($audience) && $audience ? $audience : User::find($user_id);
                      $giftReceiverName = isset($receiver_name) && $receiver_name ? $receiver_name->name : '';
                      $count = [
                        'channelName' => $channelName,
                         'name' =>  $gift->name,
                         'gift_name' => strval($newFileName),
                         'value' => strval($value),
                         'sender_id' => $giftSenderUser ? strval($giftSenderUser->id) : strval($user_id),
                         'sender_name' => $giftSenderUser ? strval($giftSenderUser->name) : '',
                         'sender_profile' => $giftSenderUser ? strval($giftSenderUser->profile) : '',
                         'receiver_name' => strval($giftReceiverName),
                         'gift_time' => strval(5),
                   ...
                   array_push($gift_effect,array('message'=>'Audio gift','channelName'=>$channelName,'data'=>$count,'code'=>'200','event_type' => 'room.gift.sent'));
      ...
      $list = array();
            ...
            $start_date = now()->startOfMonth();
              ...
              $roomName='multi_call_host_list';
               array_push($websocket_call,array('message'=>'bd_multi_call','data'=>$pusher_response,'channelName'=>$channelName,'code'=>'200','event_type' => 'multi.call.updated'));
                            self::Websoket($websocket_call);
...
function send_ws_notification($host_data, $channelName, $brd_type)
{
    ...
    $sentences = [
        ...
        "à¦†à¦®à¦¿ à¦¤à§‹à¦®à¦¾à¦° à¦œà¦¨à§à¦¯ à¦…à¦ªà§‡à¦•à§à¦·à¦¾ à¦•à¦°à¦›à¦¿, à¦¯à§‹à¦— à¦¦à¦¾à¦“ à¦à¦¬à¦‚ à¦šà¦² à¦à¦•à¦¸à¦™à§à¦—à§‡ à¦†à¦°à¦“ à¦¬à¦¨à§à¦§à§ à¦¤à§ˆà¦°à¦¿ à¦•à¦°à¦¿à¥¤",
        "Come join me, let's connect and build new friendships.",
        "à¦†à¦®à¦¾à¦° à¦¸à¦¾à¦¥à§‡ à¦¯à§‹à¦— à¦¦à¦¾à¦“, à¦šà¦² à¦¨à¦¤à§à¦¨ à¦¬à¦¨à§à¦§à§à¦¤à§à¦¬ à¦¤à§ˆà¦°à¦¿ à¦•à¦°à¦¿à¥¤",
        "Don't miss out, I'm here waiting for you to join and meet more friends.",
        "à¦®à¦¿à¦¸ à¦•à§‹à¦°à§‹ à¦¨à¦¾, à¦†à¦®à¦¿ à¦à¦–à¦¾à¦¨à§‡ à¦¤à§‹à¦®à¦¾à¦° à¦œà¦¨à§à¦¯ à¦…à¦ªà§‡à¦•à§à¦·à¦¾ à¦•à¦°à¦›à¦¿ à¦¬à¦¨à§à¦§à§à¦¦à§‡à¦° à¦¸à¦¾à¦¥à§‡ à¦¦à§‡à¦–à¦¾ à¦•à¦°à¦¾à¦° à¦œà¦¨à§à¦¯à¥¤",
        "Join me now, and let's make wonderful memories with friends.",
        "à¦à¦–à¦¨à¦‡ à¦†à¦®à¦¾à¦° à¦¸à¦¾à¦¥à§‡ à¦¯à§‹à¦— à¦¦à¦¾à¦“, à¦à¦¬à¦‚ à¦šà¦² à¦¬à¦¨à§à¦§à§à¦¦à§‡à¦° à¦¸à¦¾à¦¥à§‡ à¦šà¦®à§Žà¦•à¦¾à¦° à¦¸à§à¦®à§ƒà¦¤à¦¿ à¦¤à§ˆà¦°à¦¿ à¦•à¦°à¦¿à¥¤",
        "I'm waiting for you! Let's make our circle bigger with new friends.",
        "à¦†à¦®à¦¿ à¦¤à§‹à¦®à¦¾à¦° à¦œà¦¨à§à¦¯ à¦…à¦ªà§‡à¦•à§à¦·à¦¾ à¦•à¦°à¦›à¦¿! à¦šà¦² à¦†à¦®à¦¾à¦¦à§‡à¦° à¦¬à¦¨à§à¦§à§à¦¦à§‡à¦° à¦¸à¦‚à¦–à§à¦¯à¦¾ à¦¬à¦¾à¦¡à¦¼à¦¾à¦‡à¥¤",
        "Let's join hands and create a beautiful friendship circle.",
        "à¦šà¦² à¦¹à¦¾à¦¤à§‡ à¦¹à¦¾à¦¤ à¦°à§‡à¦–à§‡ à¦à¦•à¦Ÿà¦¿ à¦¸à§à¦¨à§à¦¦à¦° à¦¬à¦¨à§à¦§à§à¦¤à§à¦¬à§‡à¦° à¦¬à§ƒà¦¤à§à¦¤ à¦¤à§ˆà¦°à¦¿ à¦•à¦°à¦¿à¥¤",
        "Your presence will make it better, join and meet new people.",
        "à¦¤à§‹à¦®à¦¾à¦° à¦‰à¦ªà¦¸à§à¦¥à¦¿à¦¤à¦¿ à¦à¦Ÿà¦¿à¦•à§‡ à¦†à¦°à¦“ à¦¸à§à¦¨à§à¦¦à¦° à¦•à¦°à¦¬à§‡, à¦¯à§‹à¦— à¦¦à¦¾à¦“ à¦à¦¬à¦‚ à¦¨à¦¤à§à¦¨ à¦®à¦¾à¦¨à§à¦·à¦¦à§‡à¦° à¦¸à¦¾à¦¥à§‡ à¦ªà¦°à¦¿à¦šà¦¿à¦¤ à¦¹à¦“à¥¤",
        "Join me today, and let's share laughter with friends.",
        "à¦†à¦œà¦‡ à¦†à¦®à¦¾à¦° à¦¸à¦¾à¦¥à§‡ à¦¯à§‹à¦— à¦¦à¦¾à¦“, à¦à¦¬à¦‚ à¦šà¦² à¦¬à¦¨à§à¦§à§à¦¦à§‡à¦° à¦¸à¦¾à¦¥à§‡ à¦¹à¦¾à¦¸à¦¿ à¦­à¦¾à¦—à¦¾à¦­à¦¾à¦—à¦¿ à¦•à¦°à¦¿à¥¤"
    ];
