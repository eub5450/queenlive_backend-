...
class AgencyController extends Controller
{
    ...
    public function ApplyHosting(Request $request)
    {
        try {
            if (!$this->hasValidAccessToken($request)) {
                return response()->json([['message' => 'Unauthorized', 'code' => '401']], 401, [], JSON_UNESCAPED_UNICODE);
            ...
            $userId = (int) $actorId;
            $agencyCode = trim((string) ($request->agency_code ?: $request->code));
            $phoneNumber = trim((string) ($request->whatsapp ?: $request->phone_number));
            $hostingTypeRaw = $request->hosting_type;
            $hostingType = ($hostingTypeRaw === null || $hostingTypeRaw === '') ? 1 : (int) $hostingTypeRaw;
            // Only 1 (Audio) and 2 (Video) are valid hosting types. Previously any
            // out-of-range value silently fell back to Audio, which could
            // mis-file a client bug as the wrong hosting type; now reject it
            // explicitly instead of guessing. Missing/empty still defaults to
            // Audio (unchanged behavior for existing/older clients).
            if ($hostingType !== 1 && $hostingType !== 2) {
                return response()->json([['message' => 'Invalid hosting type. Must be 1 (Audio) or 2 (Video)', 'code' => '422']], 200, [], JSON_UNESCAPED_UNICODE);
            }

            if ($agencyCode === '' || $userId <= 0 || $phoneNumber === '') {
                return response()->json([['message' => 'Required field missing', 'code' => '422']], 422, [], JSON_UNESCAPED_UNICODE);
            }
            ...
            return response()->json([['message' => 'Hosting application submitted', 'code' => '200']], 200, [], JSON_UNESCAPED_UNICODE);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([['message' => 'Internal Server Error', 'code' => '500']], 500, [], JSON_UNESCAPED_UNICODE);
        }
    ...
    /**
     * "Apply Agency" self-service endpoint: an existing logged-in user
     * applies to become an agency owner. Unlike ApplyHosting (which links
     * a user to an *existing* agency via its code), this CREATES a new
     * Agency row for the caller. Only 5 fields are accepted from the
     * client per the boss's spec: id (auto/session), agency_name,
     * country (auto-derived server-side from the user's own profile,
     * never trusted from the request body), whatsapp, and nid. No file
     * uploads are required (photo_id/selfie/bank_details are left null),
     * matching the self-service mobile flow rather than the admin-panel
     * Author\AgencyController::Store flow.
     */
    public function ApplyAgency(Request $request)
    {
        try {
            ...
            $userId = (int) $actorId;
            $agencyName = trim((string) $request->agency_name);
            $phoneNumber = trim((string) ($request->whatsapp ?: $request->phone_number));
            $nid = trim((string) $request->nid);

            if ($agencyName === '' || $userId <= 0 || $phoneNumber === '' || $nid === '') {
                return response()->json([['message' => 'Required field missing', 'code' => '422']], 422, [], JSON_UNESCAPED_UNICODE);
            }
            ...
            if (Agency::where('user_id', $userId)->exists()) {
                return response()->json([['message' => 'You Have Already Applied For An Agency', 'code' => '401']], 200, [], JSON_UNESCAPED_UNICODE);
            }

            // Country is never trusted from the request — always the
            // acting user's own profile country ("auto select from id").
            $countryId = (int) ($user->country_id ?: 1);
            $agencyCode = $this->nextAgencyCode();
            ...
            return response()->json([['message' => 'Agency application submitted', 'code' => '200']], 200, [], JSON_UNESCAPED_UNICODE);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([['message' => 'Internal Server Error', 'code' => '500']], 500, [], JSON_UNESCAPED_UNICODE);
        }
    ...
    /**
     * Mirrors Author\AgencyController::nextAgencyCode() — same numbering
     * scheme (starts at AGENCY_CODE_START, next free integer code) since
     * a self-service applicant doesn't supply their own code.
     */
