...
class GlobalController extends Controller
{
    ...
    public function Index(Request $request)
    {
        ...
        if ($channelNames->isNotEmpty()) {
            ...
            // Purge feed caches so the just-removed stale room never ghosts in
            // the world/home/friends lists (parity with UserLiveController::Delete).
            $bdGhostKeys = [
                $this->prefix . 'global_lives',
                $this->prefix . 'live_users_type_1',
                $this->prefix . 'live_frined_home',
                $this->prefix . 'live_top_list',
                $this->prefix . "followerLive_{$user_id}",
            ];
            for ($bi = 1; $bi <= 10; $bi++) {
                $bdGhostKeys[] = $this->prefix . "live_list_page_{$bi}";
                $bdGhostKeys[] = $this->prefix . "live_list_v2_page_{$bi}";
            }
            foreach ($bdGhostKeys as $bdGhostKey) {
                try { Redis::del($bdGhostKey); } catch (\Throwable $e) {}
            }
        }
        ...
        $cacheLives = $this->prefix . 'global_lives';
        try {
    ...
    public function CountryWiseData(Request $request)
    {
        ...
        $cid = (int) $country_id;
        $cacheKey = $this->prefix . "global_lives_country_{$cid}";
        try {
