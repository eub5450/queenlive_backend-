...
class InviteController extends Controller
{
    ...
    public function Withdaw(Request $request)
    {
        ...
        if($token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
    		...
    		// P0-B6 FIX (Agent SF 2026-06-28): available balance read from
    		// unlocked SUMs, then withdraw-row insert + balance credit with no
    		// lock/txn (SUM-check race -> double-withdraw / double-credit).
    		// Lock the user row as mutex + recompute inside the txn.
    		$ok = DB::transaction(function () use ($user_id, $amount) {
