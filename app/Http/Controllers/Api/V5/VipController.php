        ...
        if($token=="0411f0028cfb768b3a3d96ac3aa37dw3e5"){
            if($user_id==Auth::id()){
                ...
                if(!$check_old_purchase){
                    // P0-B4 FIX (Agent SF 2026-06-28): cached UserfindById read +
                    // unlocked debit/save (lost-update). Atomic locked purchase.
            ...
            if($user_id==Auth::id()){
                ...
                try {
                    DB::transaction(function () use ($user_id, $id_number, $email) {
                        $user = User::where('id', $user_id)->lockForUpdate()->first();
                        $vip_row = VipId::where('id_number', $id_number)->lockForUpdate()->first();
                        if (!$user || !$vip_row || $user->balance <= $vip_row->price) {
                            throw new \RuntimeException('insufficient_balance');
                        }
                        $vip_row->is_purchase=1;
                        $vip_row->email=$email;
                        $vip_row->save();
                        $user->balance-=$vip_row->price;
                        $user->save();
                    });
                    $purchaseOk = true;
                } catch (\RuntimeException $e) {
