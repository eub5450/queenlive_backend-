...
class RoomCommentController extends Controller
{
    public function Send(Request $request)
    {
        ...
        // 2026-07-03 moderation fix: this is the app's primary comment path and
        // it skipped the CommentMute gate entirely - a muted user could still
        // chat to the whole room. Enforce it here like RoomActionService does.
        try {
            $isCommentMuted = \App\Models\CommentMute::where('channelName', $channelName)
                ->where('user_id', $userId)
                ->exists();
        } catch (\Throwable $muteCheckError) {
            $isCommentMuted = false;
        }
        if ($isCommentMuted) {
            array_push($response, array('message' => 'You are comment-muted in this room', 'error' => 'comment_muted', 'code' => '403'));
            return json_encode($response, JSON_UNESCAPED_UNICODE);
        }
